...
public class PerlinNoise extends FloatNoise {
	...
	public float get(double x, double y) {
		...
		a = BNVMath.lerp(dx, a, b);
		b = BNVMath.lerp(dx, c, d);
		a = BNVMath.lerp(dy, a, b);

		return BNVMath.clamp_float(a / 1.414F + 0.5F, 0.0F, 1.0F);
	}
	...
	public float get(double x, double y, double z) {
		...
		a = BNVMath.lerp(dx, a, b);
		b = BNVMath.lerp(dx, c, d);
		c = BNVMath.lerp(dx, e, f);
		d = BNVMath.lerp(dx, g, h);

		a = BNVMath.lerp(dy, a, b);
		b = BNVMath.lerp(dy, c, d);

		a = BNVMath.lerp(dz, a, b);

		return BNVMath.clamp_float(a / 1.732F + 0.5F, 0.0F, 1.0F);
	}
	...
	private void fillVector(float[] vector, int x, int y, int z) {
		...
		if (length > 0) {
			length = BNVMath.sqrt_double(length);
			vector[0] /= length;
			vector[1] /= length;
			vector[2] /= length;
		}
	...
	private void fillVector(float[] vector, int x, int y) {
		...
		vector[0] = BNVMath.sin(angle);
		vector[1] = BNVMath.cos(angle);
	}
