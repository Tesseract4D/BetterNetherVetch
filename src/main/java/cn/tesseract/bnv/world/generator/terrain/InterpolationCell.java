...
public class InterpolationCell {
	...
	public void setX(int x) {
		dx = (float) x / cellSide;
		x1 = MathHelper.clamp_int((int) dx, 0, lastIndex);
		dx -= x1;
	}

	public void setY(int y) {
		dy = (float) y / cellSide;
		y1 = MathHelper.clamp_int((int) dy, 0, lastIndex);
		dy -= y1;
	}

	public void setZ(int z) {
		dz = (float) z / cellSide;
		z1 = MathHelper.clamp_int((int) dz, 0, lastIndex);
		dz -= z1;
	}
	...
	public float get() {
		...
		for (i = 0; i < 4; i++) {
			index = i << 1;
			cell[i] = BNVMath.lerp(dz, cell[index], cell[index | 1]);
		}
		for (i = 0; i < 2; i++) {
			index = i << 1;
			cell[i] = BNVMath.lerp(dy, cell[index], cell[index | 1]);
		}
		return BNVMath.lerp(dx, cell[0], cell[1]);
	}
