...
public abstract class DataMap<T> {
    ...
    public void setData(ISaveHandler data, int seed) {
        ...
        folder = new File(data.getMapFileFromName("").getParentFile(), dataKey);
        if (!folder.exists()) {
        ...
        return chunks.computeIfAbsent(getKey(cx, cz), p -> {
            ...
            if (file.exists()) {
                try {
                    ...
                    BNV.LOGGER.warn("Failed to load region " + cx + " " + cz + " for map " + dataKey + ", reason: " + e.getLocalizedMessage());
                }
            }
            ...
            if (!loaded) {
                ...
                try {
                    ...
                    BNV.LOGGER.warn("Failed to save region " + cx + " " + cz + " for map " + dataKey + ", reason: " + e.getLocalizedMessage());
                }
            }
