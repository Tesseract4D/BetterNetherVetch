...
import cn.tesseract.bnv.noise.SphericalVoronoiNoise;
...
public class ArchipelagoFeature extends TerrainFeature {
	...
	public float getDensity(int x, int y, int z) {
		...
		if (y < 150) {
			...
			float power = BNVMath.cos(islandsPower.get(x * 0.005 * 3.0, z * 0.005 * 3.0) * PI_HALF) * 0.5F + 0.5F;
		...
		if (y > 150) {
			...
			float spikes = BNVMath.cos(ceilingSpikes.get(x * 0.02 * 3.0, z * 0.02 * 3.0) * PI_HALF) * 0.5F + 0.5F;
