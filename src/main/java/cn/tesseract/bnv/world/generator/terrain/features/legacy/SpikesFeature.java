package cn.tesseract.bnv.world.generator.terrain.features.legacy;
...
public class SpikesFeature extends TerrainFeature {
	...
	public float getDensity(int x, int y, int z) {
		...
		spikes = BNVMath.cos(this.bigSpikesFloor.get(x * 0.1, z * 0.1) * PI_HALF) * 0.5F + 0.5F;
		spikes = spikes * spikes * spikes;
		spikes += gradient(y, 0, 200 * height, 0.1F, -1.0F);
		density = Math.max(density, spikes);

		spikes = BNVMath.cos(this.smallSpikesFloor.get(x * 0.3, z * 0.3) * PI_HALF) * 0.5F + 0.5F;
		spikes = spikes * spikes * spikes;
		...
		spikes = BNVMath.cos(this.bigSpikesCeiling.get(x * 0.1, z * 0.1) * PI_HALF) * 0.5F + 0.5F;
		spikes = spikes * spikes * spikes;
		spikes += gradient(y, 256 - 200 * height, 256, -1.0F, 0.1F);
		density = Math.max(density, spikes);

		spikes = BNVMath.cos(this.smallSpikesCeiling.get(x * 0.3, z * 0.3) * PI_HALF) * 0.5F + 0.5F;
		spikes = spikes * spikes * spikes;
