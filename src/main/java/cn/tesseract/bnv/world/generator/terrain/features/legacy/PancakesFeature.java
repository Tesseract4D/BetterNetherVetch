...
public class PancakesFeature extends TerrainFeature {
	...
	public float getDensity(int x, int y, int z) {
		...
		float sin = BNVMath.sin(density * count * PI);
		...
		float ceil = BNVMath.cos(ceilingSpikes.get(x * 0.02, z * 0.02) * PI_HALF) * 0.5F + 0.5F;
		ceil = ceil * ceil * ceil;
		ceil += gradient(y, 200, 256, -1.0F, 0.5F);
		density = Math.max(density, ceil);

		float floor = BNVMath.cos(floorSpikes.get(x * 0.02, z * 0.02) * PI_HALF) * 0.5F + 0.5F;
