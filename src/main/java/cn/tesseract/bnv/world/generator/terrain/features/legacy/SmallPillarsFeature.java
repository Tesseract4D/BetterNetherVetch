...
public class SmallPillarsFeature extends TerrainFeature {
	...
	private float getPillar(int seed, Vector3d pos) {
		random.setSeed(seed);
		...
		if (y > 0) {
			y *= 2;
			density = radius - (BNVMath.sqrt_double(x * x + z * z + y * y));
		}
		else {
			density = radius - BNVMath.sqrt_double(x * x + z * z) + y;
		}

		float support = y > 0 ? 0 : radius * 0.9F - (BNVMath.sqrt_double(x * x + z * z));
