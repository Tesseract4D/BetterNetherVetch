...
public class PillarsFeature extends TerrainFeature {
	...
	public float getDensity(int x, int y, int z) {
		...
		float size = BNVMath.cos(height * 1 - 0.5F);
		...
		if (y > maxHeight) {
			...
			density = BNVMath.sqrt_double(density);
		}
		else {
			density = 1.0F - BNVMath.sqrt_double(density);
		}
		...
		float spikes = BNVMath.cos(this.spikes.get(x * 0.1, z * 0.1) * PI_HALF) * 0.5F + 0.5F;
